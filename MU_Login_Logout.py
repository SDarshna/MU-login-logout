...
def list_gp_user(user,time_range):
...
def send_post(mu_status_url,header,payload):
    ...
    for data in dataList:
        ...
        print(data)
        RList.append([data['gpuser_name'], data["client_private_address"], data["user_source_ip"],data["geoip_from_country_name"], data["login_timestamp"], logout_ts])
        index+=1
...
def go():
    ...
    parser.add_argument('-user', '--User', help='Check login/logout times for a given Mobile user ',default="all")
